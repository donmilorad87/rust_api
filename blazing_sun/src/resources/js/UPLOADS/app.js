(function(){"use strict";function v(){const d=document.querySelector('meta[name="csrf-token"]');return d?d.getAttribute("content"):(console.warn('CSRF token not found. Ensure <meta name="csrf-token"> exists in page head.'),null)}function h(d={}){const e=v(),t={"Content-Type":"application/json",...d};return e&&(t["X-CSRF-TOKEN"]=e),t}class b{constructor(e,t,s,l,a,i,o=!1){this.upload=e,this.baseUrl=t,this.onEditClick=s,this.onDeleteClick=l,this.onPreviewClick=a,this.onSelectToggle=i,this.isSelected=o}render(){const e=document.createElement("div");e.className=`asset-card${this.isSelected?" asset-card--selected":""}`,e.dataset.uuid=this.upload.uuid;const t=this.isImageType(this.upload.mime_type),s=this.isVideoType(this.upload.mime_type),l=this.upload.mime_type==="application/pdf",a=this.upload.storage_type==="public",i=a?`${this.baseUrl}/api/v1/upload/download/public/${this.upload.uuid}`:`${this.baseUrl}/api/v1/upload/private/${this.upload.uuid}`,o=this.renderPreview(t,s,l,i),n=this.upload.title||this.upload.original_name,c=this.upload.description||"",u=this.formatBytes(this.upload.size_bytes);e.innerHTML=`
    ...
    `;const x=e.querySelector(".btn--edit"),k=e.querySelector(".btn--delete"),p=e.querySelector(".asset-card__checkbox"),g=e.querySelector(".asset-card__select");return x.addEventListener("click",r=>{r.stopPropagation(),this.onEditClick(this.upload)}),k.addEventListener("click",r=>{r.stopPropagation(),this.onDeleteClick(this.upload.uuid)}),g&&g.addEventListener("click",r=>{r.stopPropagation()}),p&&(p.addEventListener("click",r=>{r.stopPropagation()}),p.addEventListener("change",r=>{r.stopPropagation(),e.classList.toggle("asset-card--selected",p.checked),this.onSelectToggle&&this.onSelectToggle(this.upload.uuid,p.checked)})),e.querySelector(".asset-card__preview").addEventListener("click",r=>{if(!r.target.closest(".btn")&&!r.target.closest(".asset-card__select"))if(this.onPreviewClick){const U=this.isImageType(this.upload.mime_type)?`${i}?variant=full`:i;this.onPreviewClick(this.upload,U)}else window.open(i,"_blank")}),e}renderPreview(e,t,s,l){return e?`<img src="${`${l}?variant=small`}" alt="${this.escapeHtml(this.upload.title||this.upload.original_name)}" class="asset-card__image" data-full-url="${l}">`:t?`
    ...
    `,this.updateBulkSelectionUI()}renderPagination(){if(!this.pagination)return;const e=Math.ceil(this.totalUploads/this.itemsPerPage);if(e<=1){this.pagination.innerHTML="";return}let t='<div class="pagination">';t+=`
