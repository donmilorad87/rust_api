(function(){"use strict";class c{constructor(t,e={}){if(!t){console.error("Pagination: Container element required");return}this.container=t,this.currentPage=e.currentPage||1,this.totalPages=e.totalPages||1,this.onPageChange=e.onPageChange||(()=>{}),this.render(),this.bindEvents()}calculatePageWindow(){let s,i;return this.totalPages<=7?(s=1,i=this.totalPages):this.currentPage<=4?(s=1,i=7):this.currentPage>=this.totalPages-3?(s=this.totalPages-7+1,i=this.totalPages):(s=this.currentPage-3,i=this.currentPage+3),{startPage:s,endPage:i}}render(){if(this.totalPages<=1){this.container.innerHTML="";return}const{startPage:t,endPage:e}=this.calculatePageWindow();let s="";for(let i=t;i<=e;i++){const a=i===this.currentPage;s+=`
    ...
    `,this.showSuggestions();try{const e=new URLSearchParams({q:t,per_page:this.previewLimit.toString(),page:"1"}),s=await fetch(`${this.baseUrl}/api/v1/blog/search?${e}`);if(!s.ok)throw new Error("Failed to fetch search results");const i=await s.json();this.suggestions=(i.results||[]).map(a=>({id:a.id,title:a.title,slug:a.slug,excerpt:a.excerpt,category:a.categories&&a.categories[0]?a.categories[0].name:null,categorySlug:a.categories&&a.categories[0]?a.categories[0].slug:null,published_at:a.published_at,highlights:a.highlights})),this.totalResults=i.pagination?.total||0,this.activeIndex=-1,this.renderSuggestions()}catch(e){console.error("SearchBox: Error fetching search results:",e),this.suggestions=[],this.suggestionsEl.innerHTML=`
      ...
      ${e}
      ${s}
    `,this.showSuggestions()}truncateText(t,e){if(!t)return"";const s=t.replace(/<[^>]*>/g,"");return s.length<=e?this.escapeHtml(s):this.escapeHtml(s.substring(0,e).trim())+"..."}highlightMatch(t,e){if(!e)return this.escapeHtml(t);const s=this.escapeHtml(t),i=new RegExp(`(${this.escapeRegex(e)})`,"gi");return s.replace(i,"<mark>$1</mark>")}updateActiveSuggestion(){const t=this.suggestionsEl.querySelectorAll(".search-widget__result");t.forEach((e,s)=>{const i=s===this.activeIndex;e.classList.toggle("search-widget__result--active",i),e.setAttribute("aria-selected",i)}),this.activeIndex>=0&&t[this.activeIndex]?.scrollIntoView({block:"nearest"})}selectSuggestion(t){const e=this.suggestions[t];e&&(this.input.value=e.title,this.hideSuggestions(),this.onSuggestionClick&&this.onSuggestionClick(e))}submitSearch(){const t=this.input.value.trim();t&&(this.hideSuggestions(),this.onSearch?this.onSearch(t):window.location.href=`${this.baseUrl}/blog/search?q=${encodeURIComponent(t)}`)}showSuggestions(){this.suggestionsEl.classList.add("search-widget__suggestions--visible")}hideSuggestions(){this.suggestionsEl.classList.remove("search-widget__suggestions--visible"),this.activeIndex=-1}setValue(t){this.input.value=t}getValue(){return this.input.value.trim()}focus(){this.input.focus()}clear(){this.input.value="",this.suggestions=[],this.hideSuggestions()}escapeHtml(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}escapeRegex(t){return t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}}class p{constructor(t){this.baseUrl=t.baseUrl||"",this.postsContainer=t.postsContainer,this.paginationContainer=t.paginationContainer,this.tagCloudContainer=t.tagCloudContainer,this.archiveContainer=t.archiveContainer,this.searchContainer=t.searchContainer,this.featuredContainer=t.featuredContainer,this.showToast=t.showToast||(()=>{}),this.currentPage=1,this.postsPerPage=9,this.totalPosts=0,this.posts=[],this.featuredPosts=[],this.isLoading=!1,this.pagination=null,this.tagCloud=null,this.archiveWidget=null,this.searchBox=null,this.init()}async init(){this.initWidgets(),await this.loadInitialData()}initWidgets(){this.paginationContainer&&(this.pagination=new c(this.paginationContainer,{currentPage:this.currentPage,totalPages:1,onPageChange:t=>this.loadPosts(t)})),this.searchContainer&&(this.searchBox=new u(this.searchContainer,{baseUrl:this.baseUrl,placeholder:"Search posts...",onSearch:t=>this.handleSearch(t),onSuggestionClick:t=>this.navigateToPost(t)})),this.tagCloudContainer&&(this.tagCloud=new g(this.tagCloudContainer,{tags:[],baseUrl:this.baseUrl})),this.archiveContainer&&(this.archiveWidget=new d(this.archiveContainer,{archives:[],baseUrl:this.baseUrl}))}async loadInitialData(){this.showLoading();try{await Promise.all([this.loadPosts(1),this.loadFeaturedPosts(),this.loadTags(),this.loadArchives()])}catch(t){console.error("BlogHomePage: Error loading initial data:",t),this.showError("Failed to load blog data. Please try again.")}}async loadPosts(t=1){this.currentPage=t,this.isLoading=!0;try{const e=new URLSearchParams({page:t.toString(),limit:this.postsPerPage.toString(),status:"published"}),s=await fetch(`${this.baseUrl}/api/v1/blog/posts?${e}`);if(!s.ok)throw new Error("Failed to load posts");const i=await s.json();if(this.posts=i.posts||[],this.totalPosts=i.pagination?.total||0,this.renderPosts(),this.pagination){const a=Math.ceil(this.totalPosts/this.postsPerPage);this.pagination.update(this.currentPage,a)}}catch(e){console.error("BlogHomePage: Error loading posts:",e),this.showToast("Failed to load posts","error"),this.renderEmpty()}finally{this.isLoading=!1}}async loadFeaturedPosts(){if(this.featuredContainer)try{const t=await fetch(`${this.baseUrl}/api/v1/blog/posts/featured?limit=2`);if(!t.ok)throw new Error("Failed to load featured posts");const e=await t.json();this.featuredPosts=e.posts||[],this.renderFeaturedPosts()}catch(t){console.error("BlogHomePage: Error loading featured posts:",t),this.featuredContainer.style.display="none"}}async loadTags(){if(this.tagCloud)try{const t=await fetch(`${this.baseUrl}/api/v1/blog/tags?limit=30`);if(!t.ok)throw new Error("Failed to load tags");const e=await t.json();this.tagCloud.update(e.tags||[])}catch(t){console.error("BlogHomePage: Error loading tags:",t)}}async loadArchives(){if(this.archiveWidget)try{const t=await fetch(`${this.baseUrl}/api/v1/blog/archives`);if(!t.ok)throw new Error("Failed to load archives");const e=await t.json();this.archiveWidget.update(e.archives||[])}catch(t){console.error("BlogHomePage: Error loading archives:",t)}}renderPosts(){if(!this.postsContainer)return;if(this.posts.length===0){this.renderEmpty();return}const t=this.posts.map(e=>this.renderPostCard(e)).join("");this.postsContainer.innerHTML=`
      <div class="posts-grid">
        ${t}
      </div>
    `}renderFeaturedPosts(){if(!this.featuredContainer)return;if(this.featuredPosts.length===0){this.featuredContainer.style.display="none";return}const t=this.featuredPosts.map(e=>this.renderPostCard(e,!0)).join("");this.featuredContainer.innerHTML=`
    ...
    `}renderPostCard(t,e=!1){const s=t.featured_image_url?`<img src="${this.escapeHtml(t.featured_image_url)}" alt="${this.escapeHtml(t.title)}" loading="lazy">`:`
      ...
      `,i=t.category?`<a href="${this.baseUrl}/blog/category/${t.category.slug}" class="post-card__category">${this.escapeHtml(t.category.name)}</a>`:"",a=t.tags&&t.tags.length>0?`
        <div class="post-card__tags">
          ${t.tags.slice(0,3).map(l=>`
            <a href="${this.baseUrl}/blog/tag/${l.slug}" class="post-card__tag">${this.escapeHtml(l.name)}</a>
          `).join("")}
        </div>
      `:"",o=this.formatDate(t.published_at||t.created_at),r=t.read_time_minutes||this.estimateReadTime(t.content);return`
      <article class="post-card ${e?"post-card--featured":""}">
        ...
        <div class="post-card__content">
          ...
          <p class="post-card__excerpt">${this.escapeHtml(t.excerpt||this.truncateText(t.content,150))}</p>
          ${a}
          <div class="post-card__meta">
            ${t.author?`
              <div class="post-card__author">
                ${t.author.avatar_url?`<img src="${this.escapeHtml(t.author.avatar_url)}" alt="" class="post-card__author-avatar">`:""}
                <span>${this.escapeHtml(t.author.display_name||t.author.first_name||"Anonymous")}</span>
              </div>
            `:""}
            <div class="post-card__date">
              ...
              <time datetime="${t.published_at||t.created_at}">${o}</time>
            </div>
    ...
    `)}showError(t){this.postsContainer&&(this.postsContainer.innerHTML=`
      <div class="blog-error">
        ...
        <p class="blog-error__text">${this.escapeHtml(t)}</p>
        <button class="blog-error__btn" data-action="retry">Try Again</button>
      </div>
    `,this.postsContainer.querySelector('[data-action="retry"]')?.addEventListener("click",()=>{this.loadInitialData()}))}handleSearch(t){window.location.href=`${this.baseUrl}/blog/search?q=${encodeURIComponent(t)}`}navigateToPost(t){t.slug&&(window.location.href=`${this.baseUrl}/blog/${t.slug}`)}formatDate(t){return t?new Date(t).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}):""}estimateReadTime(t){if(!t)return 1;const s=t.replace(/<[^>]*>/g,"").split(/\s+/).length;return Math.max(1,Math.ceil(s/200))}truncateText(t,e){if(!t)return"";const s=t.replace(/<[^>]*>/g,"");return s.length<=e?s:s.substring(0,e).trim()+"..."}escapeHtml(t){if(!t)return"";const e=document.createElement("div");return e.textContent=t,e.innerHTML}}function h(){const n=document.getElementById("postsContainer"),t=document.getElementById("pagination"),e=document.getElementById("tagCloudWidget"),s=document.getElementById("archiveWidget"),i=document.getElementById("searchWidget"),a=document.getElementById("featuredPosts");if(!n){console.error("BlogHomePage: Posts container element not found");return}const o=window.BASE_URL||"",r=v(),l=new p({baseUrl:o,postsContainer:n,paginationContainer:t,tagCloudContainer:e,archiveContainer:s,searchContainer:i,featuredContainer:a,showToast:r});typeof window<"u"&&(window.blogController=l)}function v(){const n={success:"linear-gradient(to right, #00b09b, #96c93d)",error:"linear-gradient(to right, #ff5f6d, #ffc371)",info:"linear-gradient(to right, #667eea, #764ba2)"};return function(e,s="success"){typeof Toastify<"u"?Toastify({text:e,duration:4e3,gravity:"top",position:"right",style:{background:n[s]||n.info}}).showToast():console.log(`[${s.toUpperCase()}] ${e}`)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",h):h()})();
