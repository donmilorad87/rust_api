...
function initPage() {
  const headerContainer = document.getElementById('tagHeader');
  ...
  if (!postsContainer) {
    console.error('BlogTagPage: Posts container not found');
    return;
  }

  const baseUrl = window.BASE_URL || '';
  const tagSlug = window.TAG_SLUG || '';

  const showToast = createToastFunction();

  const controller = new BlogTagPage({
    baseUrl,
    tagSlug,
    headerContainer,
    postsContainer,
  ...
  if (typeof window !== 'undefined') {
    window.blogTagController = controller;
  }
}
