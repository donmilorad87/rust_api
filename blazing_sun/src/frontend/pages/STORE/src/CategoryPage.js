 ...
 *
 * Manages the store category page with product listing, filtering, and pagination.
 * Similar to StorePage but pre-filtered by category.
 */
export class CategoryPage {
  constructor(config) {
    ...
    this.storeMain = document.getElementById('storeMain');
  ...
  async init() {
    ...
    // Load category first, then products
  ...
  async loadCategory() {
    try {
      ...
      this.category = categories.find(c => c.slug === this.categorySlug);
  ...
  renderCategory() {
    if (this.categoryTitle) {
      this.categoryTitle.textContent = this.category.name;
    }
    if (this.categoryDescription && this.category.description) {
      this.categoryDescription.textContent = this.category.description;
    }
    if (this.breadcrumbCategory) {
      this.breadcrumbCategory.textContent = this.category.name;
    }

    // Update page title
    document.title = `${this.category.name} - Photo Store`;

    // Hide loading, show header and main content
    if (this.categoryLoading) this.categoryLoading.style.display = 'none';
    if (this.categoryHeader) this.categoryHeader.style.display = 'block';
    if (this.storeMain) this.storeMain.style.display = 'grid';
  }
  ...
  showCategoryError() {
    if (this.categoryLoading) this.categoryLoading.style.display = 'none';
    if (this.categoryError) this.categoryError.style.display = 'block';
    if (this.categoryHeader) this.categoryHeader.style.display = 'none';
    if (this.storeMain) this.storeMain.style.display = 'none';
  }
  ...
  async loadProducts() {
    ...
    try {
      // Build query params - always include category_id
      const params = new URLSearchParams();
      params.append('page', this.currentPage.toString());
      params.append('limit', this.limit.toString());
      params.append('category_id', this.category.id.toString());
  ...
  showContentState() {
    ...
    if (this.errorState) this.errorState.style.display = 'none';
    if (this.emptyState) this.emptyState.style.display = 'none';
    if (this.productsGrid) this.productsGrid.style.display = 'grid';
  }
