export class MiniRouletteGame extends HTMLElement {
			constructor() {
				super();
				...
				setTimeout(() => {
					...
					// REST API endpoints
					this.endpoints = {
						spin: this.dataset.endpointSpin || '/api/v1/roulette/spin',
						history: this.dataset.endpointHistory || '/api/v1/roulette/history'
					};
...
// 		}
			...
			async fetchHistory(page = 1) {
				...
				try {
					...
					const response = await this.getRequest(this.endpoints.history, { page });
		...
		`;
			...
			async handleSpin() {
				...
				const payload = {
					bets: this.state.placements,
					bet_multiplier: this.betMultiplier || 1
				};
				...
				try {
					this.scrollWheelIntoView();
					const response = await this.postRequest(this.endpoints.spin, payload);
		...
		`;
			...
			async postRequest(url, payload = {}) {
				...
				if (!data.success) {
					const message = data?.message || data?.data?.message || 'Something went wrong.';
