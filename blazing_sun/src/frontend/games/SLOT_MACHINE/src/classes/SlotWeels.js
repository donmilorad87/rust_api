...
export default class SlotWeels extends DrawLines {
    ...
    constructor() {
        ...
        this.slotObject = {
            brojKredita: window.USER_BALANCE_COINS || 0,
            ulog: 2,
        ...
        // Initialize balance display from server-provided value
        if (this.slotObject.brojkreditadiv) {
            this.slotObject.brojkreditadiv.textContent = this.slotObject.brojKredita;
        }
    ...
    perspectiveNo() {
        ...
        for (let i = 0; i < 5; i++) {
            this.slotObject.scene[i].setAttribute('style', 'perspective: initial;transition: 0.3s;');
    ...
    rotateSlots(array) {
        ...
        this.postData('/api/games/slot-machine', sendObj)
            .then((data) => {
    ...
    async postData(url = '', data = {}) {
        // Get JWT token from window (set by template)
        const token = window.JWT_TOKEN || localStorage.getItem('jwt_token') || '';
            ...
            if (array[7] > 0) {
                ...
                if (array[9] === 0) {
                    ...
                    button2.addEventListener('click', () => {
                        // Remove the winner overlay to show mini-game
                        winwerDiv.remove();

                        // Get user's current coin balance
                        const userCoins = this.slotObject.brojKredita || 0;

                        // Create mini-game instance
                        // For legacy DOM usage, we use document.body as the parent
                        ...
                        const miniGame = new MiniGame(
                            { querySelector: (sel) => miniGameContainer.querySelector(sel), appendChild: (el) => miniGameContainer.appendChild(el), host: miniGameContainer },
                            userCoins,
                            (results) => {
                                // Cleanup mini-game container
                                miniGameContainer.remove();
                                document.getElementsByTagName("BODY")[0].style.overflow = 'initial';

                                // Update balance if we have new balance from results
                                if (results && results.new_balance !== undefined) {
                                    this.slotObject.brojKredita = results.new_balance;
                                    const kreditDiv = document.getElementById('brojKredita');
                                    if (kreditDiv) kreditDiv.textContent = results.new_balance;
                                }
                            }
                        );
