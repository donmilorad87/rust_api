...
/// Request for direct password change (current password verification)
#[derive(Deserialize, Debug)]
pub struct ChangePasswordDirectRequestRaw {
...
impl ActivationController {
    ...
    ///
    /// Verifies current password and changes to new password immediately.
    ...
    /// - 400: Validation failed (weak password, passwords don't match)
    /// - 401: Unauthorized or current password incorrect
    pub async fn change_password_direct(
        state: web::Data<AppState>,
        req: HttpRequest,
        body: web::Json<ChangePasswordDirectRequestRaw>,
    ) -> HttpResponse {
        // Get user ID from JWT
        let user_id = match req.extensions().get::<i64>() {
            Some(id) => *id,
        ...
        if raw.current_password.is_none() {
            missing_fields.push("current_password is required".to_string());
        }
        if raw.new_password.is_none() {
            missing_fields.push("new_password is required".to_string());
        }
        if raw.confirm_password.is_none() {
            missing_fields.push("confirm_password is required".to_string());
        }
        ...
        let current_password = raw.current_password.unwrap();
        let new_password = raw.new_password.unwrap();
        let confirm_password = raw.confirm_password.unwrap();
        ...
        if current_password == new_password {
            ...
            errors.insert(
                ...
                vec!["new password must be different from current password".to_string()],
            );
            return HttpResponse::BadRequest().json(ValidationErrorResponse::new(errors));
        }

        // Validate password strength
        let password_errors = validate_password(&new_password);
        if !password_errors.is_empty() {
            let mut errors: HashMap<String, Vec<String>> = HashMap::new();
            errors.insert("new_password".to_string(), password_errors);
            return HttpResponse::BadRequest().json(ValidationErrorResponse::new(errors));
        }
        ...
        if !bcrypt::verify(&current_password, &user.password).unwrap_or(false) {
            return HttpResponse::Unauthorized()
                .json(BaseResponse::error("Current password is incorrect"));
        }

        // Update password
        if let Err(e) = db_user_mutations::update_password(&db, user_id, &new_password).await {
            tracing::error!("Failed to update password: {}", e);
            return HttpResponse::InternalServerError()
        ...
        if let Some(ref mq) = state.mq {
            ...
            .with_variable("first_name", &user.first_name)
            .with_variable("email", &user.email)
            .with_variable("login_url", "/sign-in");
