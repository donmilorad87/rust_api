...
async fn test_sign_in() {
    ...
    let response: SignInResponse = serde_json::from_slice(&body)
    ...
    let user_info = vec![
        UserInfo { field: "ID".to_string(), value: response.user.id.to_string() },
        UserInfo { field: "Email".to_string(), value: response.user.email.clone() },
        UserInfo { field: "First Name".to_string(), value: response.user.first_name.clone() },
        UserInfo { field: "Last Name".to_string(), value: response.user.last_name.clone() },
        UserInfo { field: "Balance".to_string(), value: format!("{} cents", response.user.balance) },
        UserInfo { field: "Created At".to_string(), value: response.user.created_at.clone() },
    ];

    let user_table = Table::new(&user_info)
        .with(Style::rounded())
        .to_string();
    println!("{}", user_table);
    ...
    let token_info = vec![
        UserInfo { field: "Subject (sub)".to_string(), value: claims.sub.to_string() },
        UserInfo { field: "Role".to_string(), value: claims.role.clone() },
        UserInfo { field: "Expires (exp)".to_string(), value: claims.exp.to_string() },
    ];

    let token_table = Table::new(&token_info)
        .with(Style::rounded())
        .to_string();
    println!("{}", token_table);
