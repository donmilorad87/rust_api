...
This document provides comprehensive documentation for all web routes in the Money Flow application.

---

## Overview

Web routes serve HTML pages rendered using Tera templates. They are defined in `money_flow/src/routes/web.rs` and handled by the `PagesController` in `money_flow/src/app/http/web/controllers/pages.rs`.
