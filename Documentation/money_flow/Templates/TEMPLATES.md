...
This document provides comprehensive documentation for the Tera template system in the Money Flow application.

---

## Overview

The Money Flow application uses **Tera** for server-side HTML rendering. Tera is a template engine inspired by Jinja2 and Django templates.
...
<head>
    ...
    <title>{% block title %}MoneyFlow{% endblock %}</title>
...
|-------|---------|----------|
| `title` | Page title | No (default: "MoneyFlow") |
...
{% block title %}Sign In - MoneyFlow{% endblock %}
...
| `app_name` | String | Application name ("MoneyFlow") |
...
class ThemeManager {
    ...
    static setCookie(theme) {
        document.cookie = `moneyflow_theme=${theme};path=/;max-age=31536000`;
    }
}
```
...
{% block title %}My Page - MoneyFlow{% endblock %}
