...
This document provides comprehensive documentation for the database layer in the Money Flow application, including SQLx migrations, the read/mutations pattern, and compile-time checked queries.

---

## Overview

Money Flow uses **PostgreSQL** as the primary database with **SQLx** for:
...
Money Flow uses a clean separation between read operations (SELECT) and mutations (INSERT/UPDATE/DELETE).

### Directory Structure

```
money_flow/src/app/db_query/
