...
This document provides comprehensive documentation for MongoDB integration in the Money Flow application.

---

## Overview

MongoDB is integrated into the Money Flow application as a secondary database for document storage, particularly useful for:
...
pub static MONGODB: Lazy<MongoDbConfig> = Lazy::new(|| {
    ...
    let database = std::env::var("MONGO_INITDB_DATABASE")
        .unwrap_or_else(|_| "money_flow".to_string());
