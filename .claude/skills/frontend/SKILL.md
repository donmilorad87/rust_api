...
1. Updates npm dependencies (npm-check-updates)
2. Runs `npm install`
3. Runs npm builds inside Docker container (avoids sandbox issues)
4. **Automatically increments `ASSETS_VERSION`** in `.env` after successful builds
5. Reminds you to restart Docker to apply the new version

### After Building

The script automatically increments `ASSETS_VERSION`. You just need to restart Docker:

```bash
docker compose restart rust
```

**Manual version location** (if needed):
```env
# In blazing_sun/.env
